...
function OutlineEditorRange(editor, focusItem, focusOffset, anchorItem, anchorOffset, rangeAffinity) {
	if (focusItem instanceof OutlineEditorRange) {
		var range = focusItem;
		editor = range.editor;
	...
	this.editor = editor;
	this.focusItem = focusItem || null;
	this.focusOffset = focusOffset;
	this.rangeAffinity = rangeAffinity || null;
	this.anchorItem = anchorItem || null;
...
function _isValidSelectionOffset(editor, item, itemOffset) {
	if (item && editor.isVisible(item)) {
		if (itemOffset === undefined) {
			...
			return itemOffset <= item.bodyTextLength;
		}
	}
	return false;
}

Object.defineProperty(OutlineEditorRange.prototype, 'isValid', {
	get: function () {
		return (
...
Object.defineProperty(OutlineEditorRange.prototype, 'isCollapsed', {
	get: function () {
		return this.isTextMode && this.focusOffset === this.anchorOffset;
	}
});

Object.defineProperty(OutlineEditorRange.prototype, 'isUpstreamAffinity', {
	get: function () {
		return this.rangeAffinity === Constants.SelectionAffinityUpstream;
	}
});

Object.defineProperty(OutlineEditorRange.prototype, 'isItemMode', {
	get: function () {
		return this.isValid && (
			!!this.anchorItem &&
			!!this.focusItem &&
...
Object.defineProperty(OutlineEditorRange.prototype, 'isTextMode', {
	get: function () {
		return this.isValid && (
			!!this.anchorItem &&
			this.anchorItem === this.focusItem &&
...
Object.defineProperty(OutlineEditorRange.prototype, 'isReversed', {
	get: function () {
...
Object.defineProperty(OutlineEditorRange.prototype, 'focusClientRect', {
	get: function () {
		return this.clientRectForItemOffset(this.focusItem, this.focusOffset);
	}
});

Object.defineProperty(OutlineEditorRange.prototype, 'anchorClientRect', {
	get: function () {
		return this.clientRectForItemOffset(this.anchorItem, this.anchorOffset);
	}
});

OutlineEditorRange.prototype.clientRectForItemOffset = function(item, offset) {
...
OutlineEditorRange.prototype.equals = function(otherSelection) {
	return (
...
OutlineEditorRange.prototype.rangeByExtending = function(newFocusItem, newFocusOffset, newSelectionAffinity) {
...
OutlineEditorRange.prototype.rangeByModifying = function(alter, direction, granularity) {
...
OutlineEditorRange.prototype.rangeByRevalidating = function() {
	...
	var result = new OutlineEditorRange(
		this.editor,
...
OutlineEditorRange.prototype.rangeByDeleting = function(direction, granularity) {
	if (this.isTextMode) {
...
OutlineEditorRange.prototype.nextItemOffsetInDirection = function(direction, granularity, extending) {
	if (this.isItemMode) {
		switch (granularity) {
	...
	var editor = this.editor,
		...
		upstream = OutlineEditorRange.isUpstreamDirection(direction),
		next = {
	...
	if (!focusItem) {
		...
		case 'paragraphboundary':
			next.offset = upstream ? 0 : focusItem.bodyTextLength;
			break;
		...
		case 'character':
			if (upstream) {
				if (!this.isCollapsed && !extending) {
					...
					if (focusOffset > 0) {
						...
						if (prevItem) {
							...
							next.offset = prevItem.bodyTextLength;
						}
					}
				}
			} else {
				if (!this.isCollapsed && !extending) {
					...
					if (focusOffset < focusItem.bodyTextLength) {
						next.offset = focusOffset + 1;
					} else {
		...
		case 'sentence':
			...
			if (next.offset === focusOffset) {
				...
				if (nextItem) {
					var editorRange = new OutlineEditorRange(this.editor, nextItem, upstream ? nextItem.bodyTextLength : 0);
					editorRange = editorRange.rangeByModifying('move', upstream ? 'backward' : 'forward', granularity);
					next = {
	...
	if (this.isTextMode && next.offset === undefined) {
		next.offset = upstream ? 0 : next.offsetItem.bodyTextLength;
	}

	return next;
};

OutlineEditorRange.prototype.nextItemOffsetByLineFromFocus = function(focusItem, focusOffset, direction) {
	var editor = this.editor,
	...
	if (y >= (focusViewPRect.top + viewPaddingTop) && y <= (focusViewPRect.bottom - viewPaddingBottom)) {
		...
		if (nextItem) {
			...
			if (upstream) {
				...
				picked = {
					offsetItem: focusItem,
					offset: focusItem.bodyTextLength
				};
			}
		}
...
OutlineEditorRange.prototype._calculateRangeItems = function(overRideRangeItems) {
	...
	this.rangeItemsCover = Item.coverItems(rangeItems);
...
OutlineEditorRange.prototype.toString = function(indent) {
	var focusItem = this.focusItem,
		anchorItem = this.anchorItem;
...
OutlineEditorRange.isUpstreamDirection = function isUpstreamDirection(direction) {
	return direction === 'backward' || direction === 'left' || direction === 'up';
};

OutlineEditorRange.isDownstreamDirection = function isDownstreamDirection(direction) {
...
module.exports = OutlineEditorRange;